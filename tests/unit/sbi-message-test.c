...
{
	cJSON *item = NULL;
    ogs_sbi_lnode_t *entry;

	ogs_sbi_nf_profile_t *nf_profile1;
	ogs_sbi_nf_profile_t *nf_profile2;

    ogs_sbi_udr_info_t *udr_info1;
    ogs_sbi_udr_info_t *udr_info2;

    ogs_sbi_identity_range_t *identity_range1;
    ogs_sbi_identity_range_t *identity_range2;

    ogs_sbi_plmn_id_t plmn_id1[3];
    ogs_sbi_plmn_id_t *plmn_id2;
	...
	nf_profile1->nf_type = ogs_sbi_nf_type_SMF;
	nf_profile1->nf_status = ogs_sbi_nf_status_REGISTERED;

    memset(plmn_id1, 0, sizeof(plmn_id1));

    nf_profile1->plmn_list = ogs_sbi_list_create();
    ABTS_PTR_NOTNULL(tc, nf_profile1->plmn_list);

    plmn_id1[0].mcc = "111";
    plmn_id1[0].mnc = "222";
    ogs_sbi_list_add(nf_profile1->plmn_list, &plmn_id1[0]);
    plmn_id1[1].mcc = "333";
    plmn_id1[1].mnc = "444";
    ogs_sbi_list_add(nf_profile1->plmn_list, &plmn_id1[1]);
    plmn_id1[2].mcc = "555";
    plmn_id1[2].mnc = "666";
    ogs_sbi_list_add(nf_profile1->plmn_list, &plmn_id1[2]);

    nf_profile1->nsi_list = ogs_sbi_list_create();
    ogs_sbi_list_add(nf_profile1->nsi_list, "aaa");
    ogs_sbi_list_add(nf_profile1->nsi_list, "bbbbb");
    ...
    udr_info1->supi_ranges = ogs_sbi_list_create();
    ABTS_PTR_NOTNULL(tc, udr_info1->supi_ranges);

    udr_info1->gpsi_ranges = ogs_sbi_list_create();
    ...
    ogs_sbi_list_add(udr_info1->gpsi_ranges, identity_range1);

	item = ogs_sbi_nf_profile_convertToJSON(nf_profile1);
	...
	nf_profile2 = ogs_sbi_nf_profile_parseFromJSON(item);
    ...
    ogs_sbi_list_free(udr_info1->gpsi_ranges);
    ogs_sbi_list_free(udr_info1->supi_ranges);
    ogs_free(udr_info1);

    ogs_sbi_list_free(nf_profile1->plmn_list);
    ogs_sbi_list_free(nf_profile1->nsi_list);
    ogs_free(nf_profile1);

    cJSON_Delete(item);

	ogs_sbi_nf_profile_free(nf_profile2);
}
...
static void sbi_message_test2(abts_case *tc, void *data)
{
    ...
    ogs_sbi_nf_group_cond_t *nf_group_cond1;
    ogs_sbi_nf_group_cond_t *nf_group_cond2;

	ogs_sbi_smf_info_t *smf_info1;
	ogs_sbi_smf_info_t *smf_info2;
    ogs_sbi_access_type_e access_type;

    nf_group_cond1 = ogs_calloc(1, sizeof(*nf_group_cond1));
    ABTS_PTR_NOTNULL(tc, nf_group_cond1);
	nf_group_cond1->nf_type = ogs_sbi_nf_group_cond_NFTYPE_UDR;
	nf_group_cond1->nf_group_id = "group";

	item = ogs_sbi_nf_group_cond_convertToJSON(nf_group_cond1);
    ABTS_PTR_NOTNULL(tc, item);
	...
	nf_group_cond2 = ogs_sbi_nf_group_cond_parseFromJSON(item);
    ABTS_PTR_NOTNULL(tc, nf_group_cond2);

    ABTS_STR_EQUAL(tc,
            nf_group_cond1->nf_group_id, nf_group_cond2->nf_group_id);
    ABTS_INT_EQUAL(tc, ogs_sbi_nf_group_cond_NFTYPE_UDR, nf_group_cond2->nf_type);

    cJSON_Delete(item);

    ogs_sbi_nf_group_cond_free(nf_group_cond2);

    smf_info1 = ogs_calloc(1, sizeof(*smf_info1));
    ABTS_PTR_NOTNULL(tc, smf_info1);
    smf_info1->s_nssai_smf_info_list = ogs_sbi_list_create();
	smf_info1->pgw_fqdn = "PGW_FQDN";
    smf_info1->access_type = ogs_sbi_list_create();
    ABTS_PTR_NOTNULL(tc, smf_info1->access_type);

    ogs_sbi_list_add(smf_info1->access_type,
            (void*)ogs_sbi_access_type_3GPP_ACCESS);
    ogs_sbi_list_add(smf_info1->access_type,
            (void*)ogs_sbi_access_type_NON_3GPP_ACCESS);
	item = ogs_sbi_smf_info_convertToJSON(smf_info1);
    ABTS_PTR_NOTNULL(tc, item);
	...
	smf_info2 = ogs_sbi_smf_info_parseFromJSON(item);
    ...
    ABTS_PTR_NOTNULL(tc, entry);
    access_type = (ogs_sbi_access_type_e)entry->data;
    ABTS_INT_EQUAL(tc, ogs_sbi_access_type_3GPP_ACCESS, access_type);

    entry = entry->next;
    ABTS_PTR_NOTNULL(tc, entry);
    access_type = (ogs_sbi_access_type_e)entry->data;
    ABTS_INT_EQUAL(tc, ogs_sbi_access_type_NON_3GPP_ACCESS, access_type);

    ogs_sbi_list_free(smf_info1->s_nssai_smf_info_list);
    ogs_sbi_list_free(smf_info1->access_type);
    ogs_free(smf_info1);

    cJSON_Delete(item);

    ogs_sbi_smf_info_free(smf_info2);
}
...
static void sbi_message_test3(abts_case *tc, void *data)
{
	...
	ogs_sbi_nrf_info_t *nrf_info1;
	ogs_sbi_nrf_info_t *nrf_info2;

    ogs_sbi_map_t *served_pcf_info1 = NULL;
    ogs_sbi_map_t *served_pcf_info2 = NULL;
    ogs_sbi_pcf_info_t *pcf_info1;
    ogs_sbi_pcf_info_t *pcf_info2;
    ...
    served_pcf_info1 = ogs_sbi_map_create("nfInstanceId", pcf_info1);
    ABTS_PTR_NOTNULL(tc, served_pcf_info1);
    served_pcf_info2 = ogs_sbi_map_create("nfInstanceId2", pcf_info2);
    ABTS_PTR_NOTNULL(tc, served_pcf_info2);

    nrf_info1 = ogs_calloc(1, sizeof(*nrf_info1));
    ABTS_PTR_NOTNULL(tc, nrf_info1);

    nrf_info1->served_pcf_info = ogs_sbi_list_create();
    ABTS_PTR_NOTNULL(tc, nrf_info1->served_pcf_info);
    ogs_sbi_list_add(nrf_info1->served_pcf_info, served_pcf_info1);
    ogs_sbi_list_add(nrf_info1->served_pcf_info, served_pcf_info2);

	item = ogs_sbi_nrf_info_convertToJSON(nrf_info1);
    ABTS_PTR_NOTNULL(tc, item);
    ...
    ogs_sbi_map_free(served_pcf_info1);
    ogs_sbi_map_free(served_pcf_info2);
    ogs_free(pcf_info1);
    ogs_free(pcf_info2);
    ogs_sbi_list_free(nrf_info1->served_pcf_info);
    ogs_free(nrf_info1);

	nrf_info2 = ogs_sbi_nrf_info_parseFromJSON(item);
    ...
    cJSON_Delete(item);
    ogs_sbi_nrf_info_free(nrf_info2);
}
