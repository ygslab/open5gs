...
{
    ogs_sbi_udm_info_t *udm_info_local_var = ogs_sbi_malloc(sizeof(ogs_sbi_udm_info_t));
...
void ogs_sbi_udm_info_free(ogs_sbi_udm_info_t *udm_info)
{
    ...
    ogs_sbi_list_for_each(udm_info->supi_ranges, node) {
        ogs_sbi_supi_range_free(node->data);
    }
    ogs_sbi_list_free(udm_info->supi_ranges);
    ogs_sbi_list_for_each(udm_info->gpsi_ranges, node) {
        ogs_sbi_identity_range_free(node->data);
    }
    ogs_sbi_list_free(udm_info->gpsi_ranges);
    ogs_sbi_list_for_each(udm_info->external_group_identifiers_ranges, node) {
        ogs_sbi_identity_range_free(node->data);
    }
    ogs_sbi_list_free(udm_info->external_group_identifiers_ranges);
    ogs_sbi_list_for_each(udm_info->routing_indicators, node) {
        ogs_free(node->data);
    }
...
cJSON *ogs_sbi_udm_info_convertToJSON(ogs_sbi_udm_info_t *udm_info)
{
    ...
    if (udm_info->group_id) {
        if (cJSON_AddStringToObject(item, "groupId", udm_info->group_id) == NULL) {
            ogs_error("ogs_sbi_udm_info_convertToJSON() failed [group_id]");
            goto end;
        }
    }
    ...
    if (udm_info->supi_ranges) {
        ...
        if (supi_ranges == NULL) {
            ogs_error("ogs_sbi_udm_info_convertToJSON() failed [supi_ranges]");
            goto end;
        }

        ogs_sbi_lnode_t *supi_ranges_node;
        if (udm_info->supi_ranges) {
            ogs_sbi_list_for_each(udm_info->supi_ranges, supi_ranges_node) {
    ...
    if (udm_info->gpsi_ranges) {
        ...
        if (gpsi_ranges == NULL) {
            ogs_error("ogs_sbi_udm_info_convertToJSON() failed [gpsi_ranges]");
            goto end;
        }

        ogs_sbi_lnode_t *gpsi_ranges_node;
        if (udm_info->gpsi_ranges) {
            ogs_sbi_list_for_each(udm_info->gpsi_ranges, gpsi_ranges_node) {
    ...
    if (udm_info->external_group_identifiers_ranges) {
        ...
        if (external_group_identifiers_ranges == NULL) {
            ogs_error("ogs_sbi_udm_info_convertToJSON() failed [external_group_identifiers_ranges]");
            goto end;
        }

        ogs_sbi_lnode_t *external_group_identifiers_ranges_node;
        if (udm_info->external_group_identifiers_ranges) {
            ogs_sbi_list_for_each(udm_info->external_group_identifiers_ranges, external_group_identifiers_ranges_node) {
    ...
    if (udm_info->routing_indicators) {
        ...
        if (routing_indicators == NULL) {
            ogs_error("ogs_sbi_udm_info_convertToJSON() failed [routing_indicators]");
            goto end;
        }

        ogs_sbi_lnode_t *routing_indicators_node;
        ogs_sbi_list_for_each(udm_info->routing_indicators, routing_indicators_node)  {
...
ogs_sbi_udm_info_t *ogs_sbi_udm_info_parseFromJSON(cJSON *udm_infoJSON)
{
    ...
    if (group_id) {
        if (!cJSON_IsString(group_id)) {
            ogs_error("ogs_sbi_udm_info_parseFromJSON() failed [group_id]");
            goto end;
        }
    }
    ...
    if (supi_ranges) {
        ...
        if (!cJSON_IsArray(supi_ranges)) {
            ogs_error("ogs_sbi_udm_info_parseFromJSON() failed [supi_ranges]");
            goto end;
        }
    ...
    if (gpsi_ranges) {
        ...
        if (!cJSON_IsArray(gpsi_ranges)) {
            ogs_error("ogs_sbi_udm_info_parseFromJSON() failed [gpsi_ranges]");
            goto end;
        }
    ...
    if (external_group_identifiers_ranges) {
        ...
        if (!cJSON_IsArray(external_group_identifiers_ranges)) {
            ogs_error("ogs_sbi_udm_info_parseFromJSON() failed [external_group_identifiers_ranges]");
            goto end;
        }
    ...
    if (routing_indicators) {
        ...
        if (!cJSON_IsArray(routing_indicators)) {
            ogs_error("ogs_sbi_udm_info_parseFromJSON() failed [routing_indicators]");
            goto end;
        }
