...
{
    ogs_sbi_dnn_upf_info_item_t *dnn_upf_info_item_local_var = ogs_sbi_malloc(sizeof(ogs_sbi_dnn_upf_info_item_t));
...
void ogs_sbi_dnn_upf_info_item_free(ogs_sbi_dnn_upf_info_item_t *dnn_upf_info_item)
{
    ...
    ogs_sbi_list_for_each(dnn_upf_info_item->dnai_list, node) {
        ogs_free(node->data);
    }
    ogs_sbi_list_free(dnn_upf_info_item->dnai_list);
    ogs_sbi_list_for_each(dnn_upf_info_item->pdu_session_types, node) {
...
cJSON *ogs_sbi_dnn_upf_info_item_convertToJSON(ogs_sbi_dnn_upf_info_item_t *dnn_upf_info_item)
{
    ...
    if (!dnn_upf_info_item->dnn) {
        ogs_error("ogs_sbi_dnn_upf_info_item_convertToJSON() failed [dnn]");
        goto end;
    }
    ...
    if (dnn_upf_info_item->dnai_list) {
        ...
        if (dnai_list == NULL) {
            ogs_error("ogs_sbi_dnn_upf_info_item_convertToJSON() failed [dnai_list]");
            goto end;
        }

        ogs_sbi_lnode_t *dnai_list_node;
        ogs_sbi_list_for_each(dnn_upf_info_item->dnai_list, dnai_list_node)  {
    ...
    if (dnn_upf_info_item->pdu_session_types) {
        ...
        if (pdu_session_types == NULL) {
            ogs_error("ogs_sbi_dnn_upf_info_item_convertToJSON() failed [pdu_session_types]");
            goto end;
        }

        ogs_sbi_lnode_t *pdu_session_types_node;
        if (dnn_upf_info_item->pdu_session_types) {
            ogs_sbi_list_for_each(dnn_upf_info_item->pdu_session_types, pdu_session_types_node) {
...
ogs_sbi_dnn_upf_info_item_t *ogs_sbi_dnn_upf_info_item_parseFromJSON(cJSON *dnn_upf_info_itemJSON)
{
    ...
    if (!dnn) {
        ogs_error("ogs_sbi_dnn_upf_info_item_parseFromJSON() failed [dnn]");
        goto end;
    }
    ...
    if (dnai_list) {
        ...
        if (!cJSON_IsArray(dnai_list)) {
            ogs_error("ogs_sbi_dnn_upf_info_item_parseFromJSON() failed [dnai_list]");
            goto end;
        }
    ...
    if (pdu_session_types) {
        ...
        if (!cJSON_IsArray(pdu_session_types)) {
            ogs_error("ogs_sbi_dnn_upf_info_item_parseFromJSON() failed [pdu_session_types]");
            goto end;
        }
