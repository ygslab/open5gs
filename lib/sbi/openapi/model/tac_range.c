...
{
    ogs_sbi_tac_range_t *tac_range_local_var = ogs_sbi_malloc(sizeof(ogs_sbi_tac_range_t));
...
cJSON *ogs_sbi_tac_range_convertToJSON(ogs_sbi_tac_range_t *tac_range)
{
    ...
    if (tac_range->start) {
        if (cJSON_AddStringToObject(item, "start", tac_range->start) == NULL) {
            ogs_error("ogs_sbi_tac_range_convertToJSON() failed [start]");
            goto end;
        }
    }
    ...
    if (tac_range->end) {
        if (cJSON_AddStringToObject(item, "end", tac_range->end) == NULL) {
            ogs_error("ogs_sbi_tac_range_convertToJSON() failed [end]");
            goto end;
        }
    }
    ...
    if (tac_range->pattern) {
        if (cJSON_AddStringToObject(item, "pattern", tac_range->pattern) == NULL) {
            ogs_error("ogs_sbi_tac_range_convertToJSON() failed [pattern]");
            goto end;
        }
    }
...
ogs_sbi_tac_range_t *ogs_sbi_tac_range_parseFromJSON(cJSON *tac_rangeJSON)
{
    ...
    if (start) {
        if (!cJSON_IsString(start)) {
            ogs_error("ogs_sbi_tac_range_parseFromJSON() failed [start]");
            goto end;
        }
    }
    ...
    if (end) {
        if (!cJSON_IsString(end)) {
            ogs_error("ogs_sbi_tac_range_parseFromJSON() failed [end]");
            goto end;
        }
    }
    ...
    if (pattern) {
        if (!cJSON_IsString(pattern)) {
            ogs_error("ogs_sbi_tac_range_parseFromJSON() failed [pattern]");
            goto end;
        }
    }
