...
{
    ogs_sbi_n2_interface_amf_info_t *n2_interface_amf_info_local_var = ogs_sbi_malloc(sizeof(ogs_sbi_n2_interface_amf_info_t));
...
void ogs_sbi_n2_interface_amf_info_free(ogs_sbi_n2_interface_amf_info_t *n2_interface_amf_info)
{
    ...
    ogs_sbi_list_for_each(n2_interface_amf_info->ipv4_endpoint_address, node) {
        ogs_free(node->data);
    }
    ogs_sbi_list_free(n2_interface_amf_info->ipv4_endpoint_address);
    ogs_sbi_list_for_each(n2_interface_amf_info->ipv6_endpoint_address, node) {
        ogs_free(node->data);
    }
...
cJSON *ogs_sbi_n2_interface_amf_info_convertToJSON(ogs_sbi_n2_interface_amf_info_t *n2_interface_amf_info)
{
    ...
    if (n2_interface_amf_info->ipv4_endpoint_address) {
        ...
        if (ipv4_endpoint_address == NULL) {
            ogs_error("ogs_sbi_n2_interface_amf_info_convertToJSON() failed [ipv4_endpoint_address]");
            goto end;
        }

        ogs_sbi_lnode_t *ipv4_endpoint_address_node;
        ogs_sbi_list_for_each(n2_interface_amf_info->ipv4_endpoint_address, ipv4_endpoint_address_node)  {
    ...
    if (n2_interface_amf_info->ipv6_endpoint_address) {
        ...
        if (ipv6_endpoint_address == NULL) {
            ogs_error("ogs_sbi_n2_interface_amf_info_convertToJSON() failed [ipv6_endpoint_address]");
            goto end;
        }

        ogs_sbi_lnode_t *ipv6_endpoint_address_node;
        ogs_sbi_list_for_each(n2_interface_amf_info->ipv6_endpoint_address, ipv6_endpoint_address_node)  {
    ...
    if (n2_interface_amf_info->amf_name) {
        if (cJSON_AddStringToObject(item, "amfName", n2_interface_amf_info->amf_name) == NULL) {
            ogs_error("ogs_sbi_n2_interface_amf_info_convertToJSON() failed [amf_name]");
            goto end;
        }
    }
...
ogs_sbi_n2_interface_amf_info_t *ogs_sbi_n2_interface_amf_info_parseFromJSON(cJSON *n2_interface_amf_infoJSON)
{
    ...
    if (ipv4_endpoint_address) {
        ...
        if (!cJSON_IsArray(ipv4_endpoint_address)) {
            ogs_error("ogs_sbi_n2_interface_amf_info_parseFromJSON() failed [ipv4_endpoint_address]");
            goto end;
        }
    ...
    if (ipv6_endpoint_address) {
        ...
        if (!cJSON_IsArray(ipv6_endpoint_address)) {
            ogs_error("ogs_sbi_n2_interface_amf_info_parseFromJSON() failed [ipv6_endpoint_address]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(ipv6_endpoint_address_local, ipv6_endpoint_address) {
            ...
            ogs_sbi_list_add(ipv6_endpoint_addressList, ogs_strdup(ipv6_endpoint_address_local->valuestring));
        }
    }
    ...
    if (amf_name) {
        if (!cJSON_IsString(amf_name)) {
            ogs_error("ogs_sbi_n2_interface_amf_info_parseFromJSON() failed [amf_name]");
            goto end;
        }
    }
