...
smf_info_t *smf_info_create(
    ...
    smf_info_t *smf_info_local_var = malloc(sizeof(smf_info_t));
...
void smf_info_free(smf_info_t *smf_info) {
    ...
    free(smf_info->pgw_fqdn);
    list_ForEach(listEntry, smf_info->access_type) {
        free(listEntry->data);
    }
...
cJSON *smf_info_convertToJSON(smf_info_t *smf_info) {
    ...
    // smf_info->s_nssai_smf_info_list
    ...
    if(cJSON_AddStringToObject(access_type, "", (char*)access_typeListEntry->data) == NULL)
    {
        goto fail;
    }
    }
...
smf_info_t *smf_info_parseFromJSON(cJSON *smf_infoJSON){
    ...
    cJSON_ArrayForEach(access_type_local, access_type)
    {
        ...
        list_addElement(access_typeList , strdup(access_type_local->valuestring));
    }
    }
    ...
    smf_info_local_var = smf_info_create (
        ...
        pgw_fqdn ? strdup(pgw_fqdn->valuestring) : NULL,
