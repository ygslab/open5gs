...
{
    dnn_upf_info_item_t *dnn_upf_info_item_local_var = ogs_malloc(sizeof(dnn_upf_info_item_t));
...
dnn_upf_info_item_t *dnn_upf_info_item_parseFromJSON(cJSON *dnn_upf_info_itemJSON)
{
    ...
    if (dnai_list) {
        ...
        cJSON_ArrayForEach(dnai_list_local, dnai_list) {
            ...
            list_addElement(dnai_listList, ogs_strdup(dnai_list_local->valuestring));
        }
    }
