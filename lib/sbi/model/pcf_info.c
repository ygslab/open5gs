...
pcf_info_t *pcf_info_create(
    ...
    pcf_info_t *pcf_info_local_var = malloc(sizeof(pcf_info_t));
...
void pcf_info_free(pcf_info_t *pcf_info) {
    ...
    free(pcf_info->rx_diam_host);
    free(pcf_info->rx_diam_realm);
    free(pcf_info);
}
...
pcf_info_t *pcf_info_parseFromJSON(cJSON *pcf_infoJSON){
    ...
    pcf_info_local_var = pcf_info_create (
        ...
        rx_diam_host ? strdup(rx_diam_host->valuestring) : NULL,
        rx_diam_realm ? strdup(rx_diam_realm->valuestring) : NULL
        );
