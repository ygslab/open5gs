...
{
    notification_data_t *notification_data_local_var = ogs_malloc(sizeof(notification_data_t));
...
notification_data_t *notification_data_parseFromJSON(cJSON *notification_dataJSON)
{
    ...
    event_local_nonprim = notification_event_type_parseFromJSON(event);
    ...
    if (nf_profile) {
        nf_profile_local_nonprim = nf_profile_parseFromJSON(nf_profile);
    }
